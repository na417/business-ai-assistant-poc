# 業務問い合わせ AI アシスタント

## このリポジトリで示したいこと

このPoCでは、LLMに業務回答を丸投げせず、質問分類・FAQ検索・RAG・数値計算・外部API取得を分離し、回答できない場合は NO_ANSWER として安全側に倒す設計を実装しています。

特に重視した点は以下です。

- LLM非依存で処理を制御するルーティング
- RAGの距離閾値による誤回答抑制
- route / status / scores / reason を保存する観測可能性
- 評価データによる改善前後の比較

## 概要

社内問い合わせ、社内文書の検索、売上データ確認業務の軽減を想定し、
質問の種類に応じて処理を自動分岐する AI アシスタントを実装した。  
LLM に依存せず処理を制御可能な設計とし、誤回答を防ぐ判定ロジックとログ観測を備える。

## デモ

このアプリでは、ユーザーの質問内容に応じて処理ルートを切り替えます。

### 代表的な動作例

<img src="./images/demo_faq.png" alt="FAQ回答ルート" width="600">
<img src="./images/demo_numeric.png" alt="数値回答ルート" width="600">
<img src="./images/demo_no_answer.png" alt="NO_ANSWERルート" width="600">

<details>
<summary>他のデモ画面を見る</summary>

<br>

<img src="./images/demo_rag1.png" alt="RAG回答ルート1" width="600"><img src="./images/demo_rag2.png" alt="RAG回答ルート2" width="600">
<img src="./images/demo_api.png" alt="外部API回答ルート" width="600">

</details>

### 操作デモ動画

[デモ動画を見る](./assets/demo.mp4)

---

## アーキテクチャ

```
ユーザー質問
    ↓
router.py（正規表現・キーワードスコアリング・抑制語による判定）
    ↓
├─ NUMERIC  → pandas（LLM非依存の数値処理）
├─ API      → Open-Meteo（外部データのリアルタイム取得）
└─ SEARCH
       ├─ FAQ ヒット  → SQLite から回答
       └─ FAQ 未ヒット → RAG（ChromaDB + OpenAI API）で回答
```

手順・説明系の質問は FAQ をスキップして RAG へ直接遷移する。  
生成APIでエラーが発生した場合は、回答生成を行わず、検索で取得した根拠文書を直接提示するフォールバックを実装。

---

## 設計のポイント

### ① 処理の分離

質問の種類ごとに最適な処理を割り当てることで、精度とコストを両立した。

- FAQ：定型回答 → SQLite  一問一答は正規化＋一致判定で処理
- RAG：文書検索 → ChromaDB  長文マニュアルに対するセマンティック検索のため
- NUMERIC：データ計算 → pandas  数値計算をLLMに依存しないため
- API：外部データ → 天気API  外部データをリアルタイム取得するため

### ② ルーティング設計（router.py）

3 層の判定でカテゴリを決定する。

- **REGEX**：定型パターンを即時判定
- **SCORE**：キーワード加点でカテゴリを評価
- **INHIBITOR**：説明系キーワードでスコアを減点

スコア差が小さい場合や衝突時は、安全策として SEARCH へフォールバックする。

### ③ 回答制御（NO_ANSWER）

以下の条件を満たさない場合は回答しない。

- RAG の検索結果が距離閾値を満たさない
- 数値計算に必要な情報が不足
- API で地名が特定できない

### ④ ログによる観測

全ルート共通で `route / status / router_reason / router_scores` を記録。  
RAG 実行時は距離・閾値・判定結果などを補助情報としてログに付加し、  
「なぜそのルート・回答になったか」を後から追跡できる設計とした。

---

## 評価（ルーティング精度）

約60件のテストデータで検証し、調整後の正答率は **100%**（テストデータ上、設計したケースに対して）。  
誤判定は Safe Error（SEARCHへ回避）と Dangerous Error（誤ルート選択）に分類し、  
**Dangerous Error の抑制**を優先して調整した。

---

## 技術スタック

| 技術 | 用途 |
|------|------|
| Python / Streamlit | アプリ本体 |
| OpenAI API | RAGの回答生成 |
| ChromaDB | セマンティック検索 |
| SQLite | FAQ の一致判定 |
| pandas | 数値計算（LLM 非依存） |
| Open-Meteo API | 天気データのリアルタイム取得 |

---

## セットアップ

依存ライブラリをインストールします。

```bash
pip install -r requirements.txt
```

### 環境変数の設定（Windows）

このアプリでは OpenAI API を使用するため、`OpenAI_API_KEY` の設定が必要です。
`.env.example` を参考に、ローカル環境で `.env` ファイルを作成し、実際のAPIキーを設定してください。
実際のAPIキーはGitHubには公開せず、ローカル環境でのみ設定してください。
Windowsで一時的に環境変数を設定する場合は、以下を実行します。

```bash
set OPENAI_API_KEY=your_api_key
```

### 初期化・起動

```bash
python init_db.py
python build_rag.py
streamlit run app.py
```

### 評価スクリプト

```bash
python evaluate.py
```
---

## ファイル構成

```
project/
├── app.py
├── router.py
├── numeric_qa.py
├── weather_api.py
├── faq_utils.py
├── log_db.py
├── llm.py
├── build_rag.py
├── init_db.py
├── evaluate.py
├── requirements.txt
├── README.md
├── docs/
│   └── manual.txt
├── data/
│   ├── sales.csv
│   ├── eval_questions.csv
│   ├── eval_results_v1.csv
│   ├── eval_results_v2.csv
│   └── eval_results_current.csv
└── assets/
    └── demo.mp4
```

---

## 今後の改善

- ルーティングの高度化（LLM による分類との比較）
- FAQ・RAG の検索精度改善
- 評価データの拡充
- ログを活用した改善サイクルの強化

---

## 生成ファイルについて

以下のファイル・フォルダは実行時に生成されるため、GitHubには含めていません。

- faq.db
- log.db
- chroma_db/

FAQデータベースとRAG用のChromaDBは、以下のコマンドで作成できます。

```bash
python init_db.py
python build_rag.py
```
